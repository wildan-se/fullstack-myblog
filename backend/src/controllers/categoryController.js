...
const bulkDeleteCategories = asyncHandler(async (req, res) => {
  ...
  if (!categoryIds || !Array.isArray(categoryIds) || categoryIds.length === 0) {
    return sendErrorResponse(res, 400, "IDs kategori harus berupa array dan tidak boleh kosong.");
  }

  // Optional: Check if any posts are using these categories
  // For now, we'll just delete them
  const result = await Category.deleteMany({ _id: { $in: categoryIds } });

  res.json({
    message: `${result.deletedCount} kategori berhasil dihapus.`,
